...
The ADS token enables live ADS metadata sync. Without it, `sync.py --dry-run` still works.

## Smoke Test

```bash
python tools/lint.py --quiet
python tools/sync.py search "supernova remnants" --limit 1 --dry-run
printf '{"method":"search","params":{"query":"astro","limit":3}}\n' | python tools/server.py
```

## Optional Document Parsing

For `parse`, install `xparse-cli` separately if your workflow needs PDF/OCR parsing.
